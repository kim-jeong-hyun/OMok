...
#include <iostream>
#include <vector>
#include <sstream> //스트링과 스트림 기능 합침
...
vector<string> getTokens(string input, char delimiter) {
	...
	string s;
	while (getline(f, s, delimiter)) { //문자열 한줄씩 추출(추출할 입력 스트림, 입력 스트림에서 문자를 읽어드림, 줄 구분 기호)
		tokens.push_back(s);
	}
	return tokens;
}
//방에 들어온 접속자 수
int clientCountIntRoom(int roomID) {
	int count = 0;
...
void exitClient(int roomID) {
	...
	for (int i = 0; i < connections.size(); i++) {
		if (connections[i].getRoomID() == roomID) {
			ZeroMemory(sent, 256);
			sprintf(sent, "%s","[Exit]"); // Null 문자는 '\0' 문자열 포함
			send(connections[i].getClientSocket(), sent, 256, 0);
		}
	}
}
			...
			for (int i = 0; i < connections.size(); i++) {
				if (connections[i].getClientID() == client->getClientID()) {
					...
					if (connections[i].getClientID() != -1 &&
						clientCountIntRoom(connections[i].getRoomID()) == 2) {
						//남아있는 사람에게 메시지 전송
...
int main() {
	...
	int addresssLength = sizeof(serverAddress);
	while (true) {
